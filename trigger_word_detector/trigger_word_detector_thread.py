""" This program is meant to represent the simplest way to make a trigger word detector. The approach is 
    simple and involves a loop of listening to audio and then processing it. Threading is used in order to
    asynchronously process the read data in order to cut down on any missed chunks of audio which would
    have been a result of an extended processing time. There is another issue in that I had to write to disk
    because there was an iddue with wave opening a file not from disk.
...
def main():
    ...
    def threader(): # function which the second thread runs
        background = start_up() # getting the background clip
        print("-----")
        print("-----")
        ...
        while True:
            ...
            spec_len = spec_data.shape[0] # getting amount to shave off beginning of background
            background.shape = (1402,118)
            background = np.concatenate((background[spec_len:,:],spec_data)) # updating input to model
            background.shape = (1,1402,118)
            _,desc_pred = sess.run(pred,feed_dict={x:background})
            check_if_trigger(desc_pred,spec_len)

    t = threading.Thread(target = threader) # thread used to process audio
    t.daemon = True
    t.start()
